<div class="rounded-2xl border border-gray-200 bg-white pt-4 dark:border-gray-800 dark:bg-white/[0.03]">
    ...
    <div class="flex flex-col gap-2 px-5 mb-4 sm:flex-row sm:items-center sm:justify-between sm:px-6">
        <div>
            <h3 class="text-lg font-semibold text-gray-800 dark:text-white/90">Hemograma Completo</h3>
        </div>
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
            <a 
                href="{{ route('hemograma_completo.create') }}"
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-gray-200 border-y dark:border-gray-700">
                        ...
                        <th scope="col" class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Categoria</th>
                        <th scope="col" class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Unidad</th>
                        <th scope="col" class="px-4 py-3 font-normal text-gray-500 text-start text-theme-sm dark:text-gray-400">Referencia</th>
                    ...
                    <tr>
                        <td class="py-4 whitespace-nowrap">
                            <div class="flex items-center">
                                <div class="ml-4">
                                    <div class="text-sm font-medium text-gray-900 dark:text-white">{{ $hemograma['id'] }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-500 dark:text-gray-400">{{ $hemograma['nombre'] }}</div>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-500 dark:text-gray-400">{{ $hemograma->categoria->nombre }}</div>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-500 dark:text-gray-400">{{  $hemograma->unidad->nombre  }}</div>
                        </td>
                        <td class="px-4 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-500 dark:text-gray-400">{{ $hemograma->referencia  }}</div>
                        </td>
                        ...
                        <div x-data="{ dropdownOpen: false }" class="flex justify-center relative">
                            ...
                            >
                                <a 
                                    href="{{ route('hemograma_completo.edit',$hemograma['id']) }}"

                                class="flex w-full px-3 py-2 font-medium text-left text-gray-600 dark:text-gray-300 
                                        rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700" >
                                    View More ({{ $hemograma['id'] }})
                                </a>

                                <button 
                                    wire:click="deleteTransaction({{ $hemograma['id'] }})"
