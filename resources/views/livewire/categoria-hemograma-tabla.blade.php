<x-form.table-filters 
    title="Listado de Categoria de Hemograma"
    :search="$search"
    :perPage="$perPage"
            ...
            @forelse($categorias as $key  => $categoria)
                <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                    ...
                    <td class="px-4 py-1 whitespace-nowrap">
                        <div class="text-sm text-gray-700 dark:text-gray-300 font-medium">{{  $categoria['nombre']  }}</div>
                    </td>
                    <td class="px-4 py-1 text-center whitespace-nowrap text-sm font-medium">
                        <div x-data="{ dropdownOpen: false }" class="inline-block">
                            ...
                            <template x-teleport="body">
                                ...
                                >
                                    <div class="p-1">
                                        <a href="{{ route('categoria_hemograma_completo.edit', $categoria->id) }}" class="flex items-center px-3 py-2 text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg">
                                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                            Ver detalles
                                        </a>
                                        <button wire:click="confirmDelete({{ $categoria->id }})" class="flex w-full items-center px-3 py-2 text-sm text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg text-left">
